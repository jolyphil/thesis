...
source("r_scripts/functions/eastsoc.R")
...
for (i in 1:8) {
 ...
 if (any(names(round_i) == "edulvla") == F) { 
  ...
  # _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _
  # Merge with Slovenian country-specific data ----
  
  if (i %in% 1:7) {
  
  round_i <- paste0("ESS", i, "csSI.rds") %>% # get file name
    file.path("data", "temp", .) %>%
    readRDS() %>%
    left_join(round_i, ., by = c("cntry", "idno")) %>%
    mutate(wrkorg = ifelse(cntry == "SI", wrkorgsi, wrkorg)) %>%
    select(-wrkorgsi)
...
ess <- ess %>%
  dplyr::mutate(country = case_when(
    ...
    TRUE ~ countrycode(as.character(cntry), 
                       'iso2c', 
                       'iso3c',
...
# _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _
# 1989 generation | yrbrn --> gen1989 ----

ess <- ess %>%
  mutate(gen1989 = case_when(yrbrn >= 1964 & yrbrn <= 1972 ~ 1,
                             TRUE ~ 0))

attr(ess$gen1989, "label") <- "Generation 1989"
...
# _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _
# Relative age (Chap. 5) | yrbrn --> agerel ----  

# Age relative to others within the 1989 generation
ess <- ess %>%
  mutate(agerel = case_when(gen1989 == 1 ~ yrbrn - 1964,
                            TRUE ~ NA_real_))
attr(ess$agerel, "label") <- "Age, relative"
...
pc_countries <- c("ALB", "BGR", "CZE", "DEE", "EST", "HRV", "HUN", "LTU",
                  "LVA", "POL", "ROU", "RUS", "SVK", "SVN", "UKR", "XKX")

ess <- ess %>% 
  mutate(postcommunist = case_when(country %in% pc_countries ~ 1,
                                   T ~ 0),
         postcommunist = labelled(postcommunist, 
                                  labels = c("Not postcommunist" = 0, 
                                             "postcommunist" = 1),
                                  label = "Postcommunist country"))

# ______________________________________________________________________________
# Remove israel (special case) ====
...
ess_country_years <- ess %>% 
  arrange(country, year) %>% 
  group_by(country, year) %>%
  summarize()

file.path("data", "temp", "ess_country_years.rds") %>%
  saveRDS(ess_country_years, file = .)
